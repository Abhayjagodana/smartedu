...
//     try {
//       const res = await fetch(`/api/suggestion/toggle/${id}`, { method: "PATCH" });
              ...
              <tbody className="divide-y divide-gray-200">
                {suggestions.map((item, index) => (
                  <tr key={item._id} className={index % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                    ...
                    <td className="px-4 py-2">{item.topic}</td>
                    <td className="px-4 py-2">{item.suggestion}</td>
                    {/* <td className="px-4 py-2">{item.visible ? "Yes" : "No"}</td> */}
                    {/* <td className="px-4 py-2">
                      <button
                        onClick={() => toggleVisibility(item._id)}
                        className="bg-purple-700 text-white px-2 py-1 rounded-md hover:bg-purple-800"
                      >
